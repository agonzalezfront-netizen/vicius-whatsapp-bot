...
async function turno(u){ const {texto}=await generarRespuesta({menu:MENU_FALLBACK,history,userMessage:u,sesion:i===0?'nueva':'continua'});
  ...
  history.push({role:'user',content:u});history.push({role:'assistant',content:visible});i++;
  console.log('--- ['+i+'] user: '+u+'\n'+visible.slice(0,220).replace(/\n/g,' | ')+'\n');
  return visible;}
await turno('hola');
...
console.log('render?', render? 'SÍ total='+render.calc.total : 'NO');
